import { authToken, config,customer} from '../../src/config/mongoConfig';
...
afterAll(async () => {
    await dropCollections("customers");
    await dbDisconnect();
});
...
test('The PUT request returns status 200 if updated and returns 202 if the element exists', async () => {
    ...
    const customer = {
        "contactName": "Maria Mouse"
    }
    const { status: result } = await request(app)
        .put("/api/v1/cust/" + _id_put)
        .auth(token,{ type: "bearer" })
        .set("Accept", "applicatioon/json")
        .send(customer);
    expect(result).toBe(executed)
